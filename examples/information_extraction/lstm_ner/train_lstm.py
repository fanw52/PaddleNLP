...
parser.add_argument("--output_dir", default="./tmp/ner_lstm", type=str,
                    help="The output directory where the model predictions and checkpoints will be written.")
parser.add_argument("--model_name_or_path", default="bert-wwm-chinese", type=str,
...
def evaluate(model, loss_fct, metric, data_loader, label_num, mode="valid"):
    ...
    for batch in data_loader:
        logits = model(batch['input_ids'], batch['seq_len'])
...
def do_train(args):
    ...
    dropout_prob = 0.2
    vocab_size = len(gpu_tokenizer.vocab)

    model = BiLSTM(args.emb_dim, args.hidden_size, vocab_size,
                   label_num, args.vocab_path, args.padding_idx,
                   args.num_layers, dropout_prob, args.init_scale,
                   embedding_name=None)
